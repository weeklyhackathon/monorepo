import type { ProcessSubmissionParams } from '@weeklyhackathon/agents';
...
processSubmissionsRouter.post('/', async (ctx) => {
  const {
    submission
  } = ctx.request.body as ProcessSubmissionParams;
  ...
  try {
    ...
    const {
      agent: hackerAgent, config: hackerConfig
    ...
    const {
      agent: judgeAgent, config: judgeConfig
    } = await initializeAgent(
      AgentType.Judge
    );
    ...
    const {
      agent: messengerAgent, config: messengerConfig
    } = await initializeAgent(
      AgentType.Messenger
    );
